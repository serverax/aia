...
def join_waitlist():
    try:
        ...
        c = conn.cursor()
        try:
            c.execute('INSERT INTO waitlist (name, email, profession, company, created_at) VALUES (?,?,?,?,?)',
...
def list_waitlist():
    ...
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    c.execute('SELECT id, name, email, profession, company, created_at FROM waitlist ORDER BY created_at DESC')
    rows = c.fetchall()
    conn.close()
    
    users = [{'id': r[0], 'name': r[1], 'email': r[2], 'profession': r[3], 'company': r[4], 'joined': r[5]} for r in rows]
