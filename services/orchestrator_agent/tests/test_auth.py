...
def test_login_success():
    response = client.post(
        "/token", data={"username": "admin", "password": "synthetic-admin-secret"}
    )
...
def test_login_failure():
    response = client.post("/token", data={"username": "admin", "password": "wrong-password"})
    assert response.status_code == 401
...
def test_insufficient_permissions():
    ...
    login_response = client.post(
        "/token", data={"username": "analyst", "password": "analyst-dev-pass"}
    )
    ...
    async def admin_only(
        current_user=pytest.importorskip("fastapi").Security(
            pytest.importorskip("libs.auth").get_current_active_user, scopes=["admin"]
        )
    ):
        return {"ok": True}
