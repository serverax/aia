...
async def run_detailed_benchmarks():
    ...
    print(f"Cold Embedding (Batch 50): {cold_time*1000:.2f}ms total (avg {cold_time*1000/50:.2f}ms/doc)")
    print(f"Warm Embedding (Batch 50): {warm_time*1000:.2f}ms total (avg {warm_time*1000/50:.2f}ms/doc)")
    ...
    for test in integration_queries:
        ...
        print(f"Query: {test['q'][:40]}... -> Result: {res[0]['id'] if res else 'NONE'} [{'SUCCESS' if match else 'FAILED'}]")
